...
class Orders extends PbModel {
	...
	function Add($data)
	{
		...
		if (empty($data) || !is_array($data) || $total_price<=0) {
			...
			$result = $this->dbstuff->Execute("INSERT INTO {$this->table_prefix}orders (member_id,trade_no,cache_username,subject,content,total_price,created,modified) VALUE ('".$member_id."','".$order_id."','".$username."','".$subject."','".$content."','".$total_price."',".$this->timestamp.",".$this->timestamp.")");
			if ($result) {
				return $order_id;
			}else{
				return false;
