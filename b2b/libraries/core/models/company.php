	...
	'description' => array( 'required' => true),
 	...
 	function initSearch()
 	{
 		if (isset($_GET['industryid'])) {
 			...
 			$level = intval($_GET['industrylevel']);
 			if ($level==0) {
 				$level=1;
 			}
 			$this->condition[] = "Company.industry_id".$level."=".$industry_id;
 		}
 		if (isset($_GET['areaid'])) {
 			$area_id = intval($_GET['areaid']);
 			$level = intval($_GET['arealevel']);
 			if ($level==0) {
 				$level=1;
 			}
 			$this->condition[] = "Company.area_id".$level."=".$area_id;
 		}
 		if (isset($_GET['type'])) {
